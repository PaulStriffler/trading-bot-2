...
class TradeResult:
    ...
    exit_reason: str         # "tp3", "tp2_sl", "tp1_sl", "sl", "open_close"
    rr_realized: float       # weighted R multiple
    pnl_dollar: float        # at 1% risk on $10k → $100 per 1R


def simulate(trade: Trade, df_1h: pd.DataFrame, balance: float = 10_000, risk_pct: float = 1.0) -> TradeResult:
    ...
    for ts, bar in after.iterrows():
        ...
        if trade.direction == "long":
            ...
            if l <= sl:
                if hit_tp2:
                    return _result(trade, ts, "tp2_sl", realized_r + PARTIALS[2] * _r(trade.entry_price, sl, "long", risk_unit), risk_dollar)
                if hit_tp1:
                    return _result(trade, ts, "tp1_sl", realized_r + (PARTIALS[1] + PARTIALS[2]) * _r(trade.entry_price, sl, "long", risk_unit), risk_dollar)
                return _result(trade, ts, "sl", -1.0, risk_dollar)
            ...
            if hit_tp2 and h >= trade.tp3:
                ...
                return _result(trade, ts, "tp3", realized_r, risk_dollar)
        else:  # short
            if h >= sl:
                if hit_tp2:
                    return _result(trade, ts, "tp2_sl", realized_r + PARTIALS[2] * _r(trade.entry_price, sl, "short", risk_unit), risk_dollar)
                if hit_tp1:
                    return _result(trade, ts, "tp1_sl", realized_r + (PARTIALS[1] + PARTIALS[2]) * _r(trade.entry_price, sl, "short", risk_unit), risk_dollar)
                return _result(trade, ts, "sl", -1.0, risk_dollar)
    ...
    return _result(trade, last_ts, "open_close", realized_r, risk_dollar)
...
def _result(trade: Trade, exit_time, reason: str, realized_r: float, risk_dollar: float) -> TradeResult:
    return TradeResult(
        symbol=trade.symbol,
